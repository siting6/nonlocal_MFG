function a = idct2_simple(arg1,mrows,ncols)
...
[m, n] = size(arg1);
%allocate space
a = complex(zeros(size(arg1)));
% Basic algorithm.
if (nargin == 1),
  if (m > 1) & (n > 1),
    a =idct_simple(idct_simple(arg1).').';
    return;
  else
    mrows = m;
    ncols = n;
...
a = idct_simple(b, mpad);
if m > 1 & n > 1, a =idct_simple(a.', npad).'; end